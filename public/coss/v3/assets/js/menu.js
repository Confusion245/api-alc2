try {
	...
	function requestmenu() {
		...
		Toast.fire({
			icon: 'info',
			title: '<p style="color: gray;">awa de uwu <a style="color: blue;" href="https://wa.me/51923568749">owner.</a></p>'
		})
	}
	...
	repeatinfo = setInterval(function() {
		information.innerHTML = `<b>Nombre de código del navegador:</b> ${navigator.appCodeName}<br><b>Navegador:</b> ${navigator.appName}<br><b>Cookies Activado:</b> ${navigator.cookieEnabled}<br><b>Navegador Online:</b> ${navigator.onLine}<br><b>Platforma:</b> ${navigator.platform} <br>
	...
	function time(time) {
		...
		if (minggu_str > 0 && minggu_str < 7) {
			return `<small>${minggu_str} semanas</small>`;
		} else if (minggu > 52.1428571 && minggu_str > 7) {
			return `<small>Hace unos pocos años</small>`;
		} else if (detik < 60 && detik_str > 0) {
			return `<small>${detik_str} segundos</small>`;
		} else if (menit < 60 && menit_str > 0) {
			return `<small>${menit_str} minutos</small>`;
		} else if (jam < 24 && jam_str > 0) {
			return `<small>${jam_str} horas</small>`;
		} else if (hari < 7 && hari_str > 0) {
			return `<small>${hari_str} dias</small>`;
		} else {
			return `<small>${hari_str} dias</small>`;
		}
	}
	...
	function net() {
		fetch('https://hadi-api.herokuapp.com/api/ip').then(res=>res.json()).then(res=> {
			res = res.result;
			tag_netinfo.innerHTML = `<b>IP: </b>${res.ip}<br><b>ISP: </b>${res.isp}<br><b>Ciudad: </b>${res.city}<br><b>DISTRITO: </b>${res.district}<br><b>TIMEZONE: </b>${res.timezone}<br><b>Pais: </b>${res.country}<br><b>VPN: </b>${res.isProxy ? 'true': 'false'}<br><b>LATITUD: </b>${res.latitude}<br><b>LONGITUD: </b>${res.longitude}`;
		});
	}
	net();
	try {
		...
		swal.fire({
			...
			if (result.isConfirmed) {
				swal.fire({
					title: 'Coloque su link de Youtube !',
					input: 'url',
					inputAttributes: {
						autocapitalize: 'off',
